...
SRC_URI="https://github.com/ROCm-Developer-Tools/hipamd/archive/refs/tags/rocm-${PV}.tar.gz -> rocm-hip-amd-${PV}.tar.gz
...
src_prepare() {
	...
	sed -e "/FLAGS .= \" -isystem \$HSA_PATH/d" \
		...
		-e "/rpath/s,--rpath=[^ ]*,," \
		-i ${WORKDIR}/HIP-rocm-${PV}/bin/hipcc || die
