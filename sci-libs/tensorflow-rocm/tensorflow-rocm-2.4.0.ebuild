...
SRC_URI="https://github.com/ROCmSoftwarePlatform/tensorflow-upstream/archive/r2.4-rocm-enhanced.tar.gz -> ${P}.tar.gz
...
src_install() {
	...
	do_install() {
		...
		rm -f "${D}/$(python_get_sitedir)"/${PN%%-rocm}/lib${PN%%-rocm}_framework.so* || die
		rm -f "${D}/$(python_get_sitedir)"/${PN%%-rocm}_core/lib${PN%%-rocm}_framework.so* || die
		python_optimize
	}
	...
	ebazel shutdown
	insinto /usr/include/${PN%%-rocm}/
	doins -r bazel-bin/tensorflow/include/*

	einfo "Installing libs"
	# Generate pkg-config file
	${PN%%-rocm}/c/generate-pc.sh --prefix="${EPREFIX}"/usr --libdir=$(get_libdir) --version=${MY_PV} || die
	insinto /usr/$(get_libdir)/pkgconfig
	doins ${PN%%-rocm}.pc ${PN%%-rocm}_cc.pc
