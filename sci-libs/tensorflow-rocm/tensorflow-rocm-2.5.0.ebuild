...
MY_PV=${PV/_rc/-rc}
MY_P=${PN%%-rocm}-upstream-r${MY_PV%.*}-rocm-enhanced

inherit bazel check-reqs distutils-r1 flag-o-matic prefix toolchain-funcs
...
PATCHES=(
	"${FILESDIR}"/0001-ci_build-update-check_system_libs-for-numbered-works.patch
	"${FILESDIR}"/0002-Add-use_default_shell_env-True-to-all-ctx.actions.ru.patch
	"${FILESDIR}"/0003-Add-use_default_shell_env-to-ctx.actions.run.patch
	"${FILESDIR}"/0004-Systemlib-rules-currently-can-t-handle-the-systemlib.patch
	"${FILESDIR}"/0005-WORKSPACE-add-rules-docker-http_archive-bazel-toolch.patch
	"${FILESDIR}"/0006-Relax-setup.py-version-requirements.patch
	"${FILESDIR}"/0007-example_parsing_ops-compatibility-with-protobuf-3.16.patch
)
...
SRC_URI="https://github.com/ROCmSoftwarePlatform/tensorflow-upstream/archive/r2.5-rocm-enhanced.tar.gz -> ${P}.tar.gz
		${bazel_external_uris}"
...
src_install() {
	...
	tensorflow/c/generate-pc.sh --prefix="${EPREFIX}"/usr --libdir=$(get_libdir) --version=${MY_PV} || die
	insinto /usr/$(get_libdir)/pkgconfig
	doins tensorflow.pc tensorflow_cc.pc
