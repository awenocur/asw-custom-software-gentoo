...
if [[ ${PV} == 9999 ]]; then
	...
	SRC_URI="https://archive.mesa3d.org/${MY_P%%r}.tar.xz"
...
PATCHES=( "${FILESDIR}/fix_lp_bld_misc_version_check.patch"
	  "${FILESDIR}/fix_ac_llvm_helper_version_check.patch"
	  )

MYMESONARGS=( --prefix="${EPREFIX}/usr/lib/mesa-roc"
	      --localstatedir="${EPREFIX}/usr/lib/mesa-roc/var/lib"
	      )
...
IUSE="${IUSE_VIDEO_CARDS}
	...
	lm-sensors osmesa selinux test unwind vaapi valgrind vdpau vulkan
...
pkg_setup() {
	...
	export PATH=/usr/lib/llvm/roc/bin:/bin:/usr/bin
	...
	if use gallium; then
		...
		if use video_cards_r600 ||
		   ...
		   use video_cards_nouveau; then
			...
			use vaapi && emesonargs+=( -Dva-libs-path="${EPREFIX}"/usr/lib/mesa-roc/va/drivers )
		else
